...
public interface IssueEmbeddingRepository extends ReactiveCrudRepository<IssueEmbeddingEntity, UUID> {
    ...
    @Query("SELECT ie.id, ie.finding_id, ie.content, ie.model_name, ie.created_at, " +
            "GREATEST(0, 1.0 - (ie.embedding <-> CAST(:queryEmbedding AS vector))) AS similarity " +
            "FROM issue_embeddings ie WHERE ie.finding_id != :excludeId " +
            "ORDER BY similarity DESC LIMIT :limit")
    Flux<SimilarEmbeddingRow> findSimilar(String queryEmbedding, int limit, UUID excludeId);
